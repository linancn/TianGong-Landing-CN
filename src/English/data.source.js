import React from 'react';
export const Banner10DataSource = {
  wrapper: { className: 'banner1' },
  BannerAnim: {
    children: [
      {
        name: 'elem0',
        BannerElement: { className: 'banner-user-elem' },
        textWrapper: { className: 'banner1-text-wrapper' },
        bg: { className: 'bg bg0 l4s7mvvpstt-editor_css' },
        title: {
          className: 'banner1-title l4s7mswkkdf-editor_css',
          children:
            'https://tian-gong-initiative-open.oss-cn-shenzhen.aliyuncs.com/image/%E5%A4%A9%E5%B7%A5%E7%AB%96%E5%9B%BE-%E5%8F%8D%E7%99%BD%E7%B4%AB.svg',
        },
        content: {
          className: 'banner1-content l4s7qxy68et-editor_css',
          children: (
            <span>
              <span>
                <span>
                  <span>
                    <p>
                      A shared, co-built and co-governed LCA&nbsp;ecosystem of
                      data, software and knowledge&nbsp;
                    </p>
                  </span>
                </span>
              </span>
            </span>
          ),
        },
        button: {
          className: 'banner1-button l4s7t0se7f-editor_css',
          children: 'Learn More',
        },
      },
    ],
  },
};
export const Content50DataSource = {
  wrapper: { className: 'home-page-wrapper content5-wrapper' },
  page: { className: 'home-page content5' },
  OverPack: { playScale: 0.3, className: '' },
  titleWrapper: {
    className: 'title-wrapper',
    children: [
      {
        name: 'title',
        children: (
          <span>
            <p>TianGong Database And Toolbox</p>
          </span>
        ),
        className: 'title-h1 l4s8n80hqh-editor_css',
      },
    ],
  },
  block: {
    className: 'content5-img-wrapper',
    gutter: 16,
    children: [
      {
        name: 'block0',
        className: 'block',
        md: 6,
        xs: 24,
        children: {
          wrapper: { className: 'content5-block-content' },
          img: {
            children:
              'https://tian-gong-initiative-open.oss-cn-shenzhen.aliyuncs.com/image/%E5%9B%BE%E6%A0%871-%E6%95%B0%E6%8D%AE%E5%BA%93.svg',
            className: 'l4s8n34w2p-editor_css',
          },
          content: {
            children: (
              <span>
                <span>
                  <span>
                    <span>
                      <span>
                        <p>DataBase</p>
                        <p>
                          <br />
                        </p>
                      </span>
                    </span>
                  </span>
                </span>
              </span>
            ),
            className: 'l4s8kh5n3c8-editor_css',
          },
        },
      },
      {
        name: 'block1',
        className: 'block',
        md: 6,
        xs: 24,
        children: {
          wrapper: { className: 'content5-block-content' },
          img: {
            children:
              '/LCA',
            className: 'l4s8nufa6tj-editor_css',
          },
          content: {
            children: (
              <span>
                <span>
                  <p>LCA Tools</p>
                  <p>
                    <br />
                  </p>
                </span>
              </span>
            ),
            className: 'l4s8nknz8cl-editor_css',
          },
        },
      },
      {
        name: 'block2',
        className: 'block',
        md: 6,
        xs: 24,
        children: {
          wrapper: { className: 'content5-block-content' },
          img: {
            children:
              'https://tian-gong-initiative-open.oss-cn-shenzhen.aliyuncs.com/image/%E5%9B%BE%E6%A0%871-%E7%A2%B32.svg',
            className: 'l4s8ovkn2z9-editor_css',
          },
          content: {
            children: (
              <span>
                <span>
                  <p>Footprint Calculation Tools</p>
                  <p>
                    <br />
                  </p>
                </span>
              </span>
            ),
            className: 'l4s8oiz8sl-editor_css',
          },
        },
      },
      {
        name: 'block3',
        className: 'block',
        md: 6,
        xs: 24,
        children: {
          wrapper: { className: 'content5-block-content' },
          img: {
            children:
              'https://tian-gong-initiative-open.oss-cn-shenzhen.aliyuncs.com/image/%E5%9B%BE%E6%A0%871-%E8%AE%BE%E8%AE%A1.svg',
            className: 'l4s8rhwqt3b-editor_css',
          },
          content: {
            children: (
              <span>
                <span>
                  <p>Product Ecological Design AIDS</p>
                </span>
              </span>
            ),
            className: 'l4s8rezonf8-editor_css',
          },
        },
      },
      {
        name: 'block4',
        className: 'block',
        md: 6,
        xs: 24,
        children: {
          wrapper: { className: 'content5-block-content' },
          img: {
            children:
              'https://tian-gong-initiative-open.oss-cn-shenzhen.aliyuncs.com/image/%E5%9B%BE%E6%A0%871-%E7%BD%91.svg',
            className: 'l4s8uw7vjug-editor_css',
          },
          content: {
            children: (
              <span>
                <p>
                  <span>Enterprise environmental management tools</span>
                </p>
              </span>
            ),
            className: 'l4s8t5dm1au-editor_css',
          },
        },
      },
    ],
  },
};
export const Content120DataSource = {
  wrapper: { className: 'home-page-wrapper content12-wrapper' },
  page: { className: 'home-page content12' },
  OverPack: { playScale: 0.3, className: '' },
  titleWrapper: {
    className: 'title-wrapper',
    children: [
      {
        name: 'title',
        children: (
          <span>
            <span>
              <p>Sponsored Unit</p>
            </span>
          </span>
        ),
        className: 'title-h1',
      },
    ],
  },
  block: {
    className: 'img-wrapper',
    children: [
      {
        name: 'block0',
        className: 'block',
        md: 8,
        xs: 24,
        children: {
          wrapper: { className: 'block-content' },
          img: {
            children:
              'https://tian-gong-initiative-open.oss-cn-shenzhen.aliyuncs.com/image/%E5%90%88%E4%BD%9C%E5%8D%95%E4%BD%8Dlogo-%E6%B8%85%E5%8D%8E%E5%A4%A7%E5%AD%A6.svg',
          },
        },
      },
      {
        name: 'block1',
        className: 'block',
        md: 8,
        xs: 24,
        children: {
          wrapper: { className: 'block-content' },
          img: {
            children:
              'https://tian-gong-initiative-open.oss-cn-shenzhen.aliyuncs.com/image/%E5%90%88%E4%BD%9C%E5%8D%95%E4%BD%8Dlogo-%E4%B8%AD%E5%9B%BD%E7%A7%91%E5%AD%A6%E9%99%A2.svg',
          },
        },
      },
      {
        name: 'block2',
        className: 'block',
        md: 8,
        xs: 24,
        children: {
          wrapper: { className: 'block-content' },
          img: {
            children:
              'https://tian-gong-initiative-open.oss-cn-shenzhen.aliyuncs.com/image/%E5%90%88%E4%BD%9C%E5%8D%95%E4%BD%8Dlogo-%E5%A4%8D%E6%97%A6%E5%A4%A7%E5%AD%A6.svg',
          },
        },
      },
      {
        name: 'block3',
        className: 'block',
        md: 8,
        xs: 24,
        children: {
          wrapper: { className: 'block-content' },
          img: {
            children:
              'https://tian-gong-initiative-open.oss-cn-shenzhen.aliyuncs.com/image/%E5%90%88%E4%BD%9C%E5%8D%95%E4%BD%8Dlogo-%E5%8D%97%E6%98%8C%E5%A4%A7%E5%AD%A6.svg',
          },
        },
      },
    ],
  },
};
